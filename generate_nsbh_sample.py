...
# 4 - generate mass of remnant
# 5 - save everything in a way that is useful for the analysis code
# QUESTIONS
# 1 - AW's 50% duty cycle
# 2 - _ns times (nanoseconds? but i don't know what they are) are they the 
#     noninteger part of the start time?
# 3 - LAL's volume seems to be increasing like there's no q_0
...
lal_cmd = 'lalapps_inspinj --t-distr exponential ' + \
          ...
          '--min-distance 1 --max-distance {:.1f} '.format(d_max * 1000.0) + \
          ...
          '--output ' + xml_file + ' --taper start --seed {:d}'.format(seed)
os.system(lal_cmd)

# parse into a sane format. find the data table. figure out the indices of 
# the columns we want to store.
...
fig, axes = mp.subplots(3, 3, figsize=(10, 10))
